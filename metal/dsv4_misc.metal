...
// Prefill specialization that stages each K/V row ONCE for all 64 heads.
//
// The heads8 kernel dispatches one threadgroup per (token, 8-head group), so
// every raw and selected row is fetched from device memory n_head/8 = 8 times
// per token. At short context the compressed cache still lives in the cache
// hierarchy and the refetches are cheap; past ~16k tokens of context the
// per-layer cache outgrows it and the attend stage's cost doubles for the
// same amount of arithmetic. Here one 1024-thread threadgroup owns the whole
// token: 32 simdgroups each carry TWO heads (sg and sg+32), rows are staged
// cooperatively in batches of sixteen, and every staged row feeds all 64
...
kernel void kernel_dsv4_indexed_mixed_attention_heads64_shared(
    ...
    const uint head_a = (uint)sg;
    const uint head_b = (uint)sg + 32u;
    const bool has_a = head_a < args.n_head;
    const bool has_b = head_b < args.n_head;

    device const float4 *qa4 = (device const float4 *)(q +
        (uint64_t)token * args.q_token_stride +
        (uint64_t)head_a * args.q_head_stride);
    device const float4 *qb4 = (device const float4 *)(q +
        (uint64_t)token * args.q_token_stride +
        (uint64_t)head_b * args.q_head_stride);
    const half4 qa0 = has_a ? (half4)qa4[lane +  0] : half4(0.0h);
    const half4 qa1 = has_a ? (half4)qa4[lane + 32] : half4(0.0h);
    const half4 qa2 = has_a ? (half4)qa4[lane + 64] : half4(0.0h);
    const half4 qa3 = has_a ? (half4)qa4[lane + 96] : half4(0.0h);
    const half4 qb0 = has_b ? (half4)qb4[lane +  0] : half4(0.0h);
    const half4 qb1 = has_b ? (half4)qb4[lane + 32] : half4(0.0h);
    const half4 qb2 = has_b ? (half4)qb4[lane + 64] : half4(0.0h);
    const half4 qb3 = has_b ? (half4)qb4[lane + 96] : half4(0.0h);
    ...
    if (first <= last) {
        for (uint pos0 = first; pos0 <= last; pos0 += 16u) {
            const uint n_rows = min(16u, last - pos0 + 1u);
            for (uint off = (uint)tid; off < n_rows * 128u; off += 1024u) {
                const uint r = off >> 7;
                const uint c = off & 127u;
                const uint logical = pos0 + r - first_raw_pos;
                const uint row = (args.raw_start + logical) % args.raw_cap;
            ...
            for (uint r = 0; r < n_rows; r++) {
                if (has_a) {
                    dsv4_attend_shared_h4_row_at(kv_shared, r,
                        qa0, qa1, qa2, qa3, args.scale, lane,
                        Ma, Sa, ao0, ao1, ao2, ao3);
                }
                if (has_b) {
                    dsv4_attend_shared_h4_row_at(kv_shared, r,
                        qb0, qb1, qb2, qb3, args.scale, lane,
                        Mb, Sb, bo0, bo1, bo2, bo3);
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (has_a) {
        dsv4_attend_sink(((device const float *)sinks)[head_a], Ma, Sa, ao0, ao1, ao2, ao3);
        const float ia = Sa == 0.0f ? 0.0f : 1.0f/Sa;
        device float4 *da = (device float4 *)(dst +
            (uint64_t)token * args.dst_token_stride +
            (uint64_t)head_a * args.dst_head_stride);
        da[lane +  0] = ao0 * ia;
        da[lane + 32] = ao1 * ia;
        da[lane + 64] = ao2 * ia;
        da[lane + 96] = ao3 * ia;
    ...
    if (has_b) {
        dsv4_attend_sink(((device const float *)sinks)[head_b], Mb, Sb, bo0, bo1, bo2, bo3);
        const float ib = Sb == 0.0f ? 0.0f : 1.0f/Sb;
        device float4 *db = (device float4 *)(dst +
            (uint64_t)token * args.dst_token_stride +
            (uint64_t)head_b * args.dst_head_stride);
        db[lane +  0] = bo0 * ib;
        db[lane + 32] = bo1 * ib;
        db[lane + 64] = bo2 * ib;
        db[lane + 96] = bo3 * ib;
